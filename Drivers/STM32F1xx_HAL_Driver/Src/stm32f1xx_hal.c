...
HAL_StatusTypeDef HAL_Init(void)
{
  ...
  //启用预取缓冲区，24MHZ-72MHZ必须要设置，否则会卡死
...
__weak HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority)
{
  ...
  //检查是是否超过了STK_LOAD\STK_VAL寄存器允许的最大值，24位
  //主要使用三个寄存器 STK_CTRL、STK_LOAD、STK_VAL
  //Cortex-m3 编程手册 第152页
  ...
  //检查TickPriority优先级是否超过了优先级配置的位数(4位用于抢占优先级)
  if (TickPriority < (1UL << __NVIC_PRIO_BITS))
  {
    //配置SCB_SHPR3寄存器的[31:24]位，该位是SysRick的优先级
